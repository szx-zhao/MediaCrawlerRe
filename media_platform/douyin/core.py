    ...
    async def search(self) -> None:
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while (page - start_page + 1) * dy_limit_count <= config.CRAWLER_MAX_NOTES_COUNT:
                ...
                utils.logger.info(f"[DouYinCrawler.search] Sleeping for {random.uniform(1, config.CRAWLER_MAX_SLEEP_SEC)} seconds after page {page-1}")
    ...
    async def get_aweme_detail(self, aweme_id: str, semaphore: asyncio.Semaphore) -> Any:
        ...
        async with semaphore:
            try:
                ...
                await asyncio.sleep(random.uniform(1, config.CRAWLER_MAX_SLEEP_SEC))
                utils.logger.info(f"[DouYinCrawler.get_aweme_detail] Sleeping for {random.uniform(1, config.CRAWLER_MAX_SLEEP_SEC)} seconds after fetching aweme {aweme_id}")
                return result
